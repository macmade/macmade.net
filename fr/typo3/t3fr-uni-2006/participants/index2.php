<?php
	...
	while($row = mysql_fetch_assoc($res)) {
		...
		$user[] = utf8_decode($row['company']);
		$user[] = utf8_decode($row['address']);
		$user[] = utf8_decode($row['city']);
		$user[] = utf8_decode($row['zip']);
		$user[] = utf8_decode($row['country']);
		$user[] = getVal($row['dates'],$row['country'],1);
		$user[] = getVal($row['lunch'],$row['country'],2);
		$user[] = getVal($row['dates'],$row['country'],3) . ' ' . (getVal($row['dates'],$row['country'],1,1) + getVal($row['lunch'],$row['country'],2,1));
		
		$users[] = implode(chr(9),$user);
	}
